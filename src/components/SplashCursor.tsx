...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    const blit = (() => {
      ...
      if (!buffer || !elemBuffer) return (_target: FBO | null, _doClear = false) => {};
    ...
    const createFBO = (
      ...
      if (!texture || !fbo) {
        throw new Error('Failed to create framebuffer objects.');
      }
    ...
    const getCanvasPosition = (clientX: number, clientY: number) => {
      ...
      return {
        isInside,
        posX: scaleByPixelRatio(localX),
        posY: scaleByPixelRatio(localY),
      };
    };

    const handleMouseDown = (e: MouseEvent) => {
      const pointer = pointers[0];
      const { isInside, posX, posY } = getCanvasPosition(e.clientX, e.clientY);
      if (positionMode === 'absolute' && !isInside) return;
    ...
    const handleTouchStart = (e: TouchEvent) => {
      ...
      for (let i = 0; i < touches.length; i++) {
        const { isInside, posX, posY } = getCanvasPosition(touches[i].clientX, touches[i].clientY);
        if (positionMode === 'absolute' && !isInside) continue;
  ...
  return (
    <div
      ...
      style={{
        ...
        zIndex: 220,
        pointerEvents: 'none',
        mixBlendMode: AUTO_CONTRAST ? 'difference' : 'normal',
        ...containerStyle,
      }}
      aria-hidden="true"
    >
